...
# --- GIF ANIMADO ---
STREAMER="/recalbox/share/userscripts/pixel_stream.py"
STOP_FLAG="/tmp/retropixel_marquee.stop"


# 1. GENERAMOS EL DECODIFICADOR PYTHON ON-THE-FLY (para las imágenes estáticas)
cat << 'EOF' > /tmp/marquesina_sender.py
    ...
    case "$EVENTO" in
        systembrowsing|gamelistbrowsing|rungame)
            if [ -f "$STATE_FILE" ]; then
                ...
                # A. Si estamos en el carrusel de sistemas, vamos a la carpeta única de logos
                if [ "$EVENTO" = "systembrowsing" ]; then
                    BMP_FILE="$LOGOS_DIR/$SISTEMA.bmp"
                else
                    # B. Si es un juego, miramos si es una carpeta
                    ...
                    if [ "$IS_FOLDER" = "1" ] || [ -z "$GAME_PATH" ]; then
                        # Si entras en una carpeta, mostramos el logo del sistema
                        BMP_FILE="$LOGOS_DIR/$SISTEMA.bmp"
                    else
                ...
                # 1. Si buscó un juego y no lo encontró, cae al logo del sistema
                if [ ! -f "$BMP_FILE" ]; then
                    BMP_FILE="$LOGOS_DIR/$SISTEMA.bmp"
                fi
                ...
                # Ejecutamos el envío de la imagen estática de forma asíncrona (&)
                if [ -n "$SISTEMA" ]; then
                    python3 /tmp/marquesina_sender.py "$IP_ESP32" "$PORT_ESP32" "$BMP_FILE" &
                fi

                # --- LOG TEMPORAL DE DIAGNÓSTICO: quitar en cuanto quede localizado el fallo ---
                echo "[$(date '+%H:%M:%S')] EVENTO=$EVENTO SISTEMA=$SISTEMA GAME_PATH=$GAME_PATH IS_FOLDER=$IS_FOLDER JUEGO_LIMPIO=$JUEGO_LIMPIO" >> /tmp/retropixel_debug.log

                # C. Solo al LANZAR el juego (no al navegar): si hay gif(s) para este rom,
                #    lanzamos el streamer en segundo plano. Si no hay ninguno, no pasa
                #    nada: la imagen estática de arriba se queda tal cual en el panel.
                if [ "$EVENTO" = "rungame" ] && [ -n "$JUEGO_LIMPIO" ]; then
                    rm -f "$STOP_FLAG"
                    CARPETA_SISTEMA="$BASE_DIR/$SISTEMA"
                    echo "[$(date '+%H:%M:%S')] Lanzando streamer: CARPETA=$CARPETA_SISTEMA ROM=$JUEGO_LIMPIO" >> /tmp/retropixel_debug.log
                    python3 "$STREAMER" "$IP_ESP32" "$PORT_ESP32" "$CARPETA_SISTEMA" "$JUEGO_LIMPIO" "$STOP_FLAG" >> /tmp/retropixel_debug.log 2>&1 &
                fi
            fi
            ;;

        endgame|stop|shutdown|reboot)
            # Avisamos al streamer de gif (si hay uno activo) para que se pare solo
            touch "$STOP_FLAG"
            sleep 1
            ...
            python3 /tmp/marquesina_sender.py "$IP_ESP32" "$PORT_ESP32" "STOP" &
            ;;
    esac
done
