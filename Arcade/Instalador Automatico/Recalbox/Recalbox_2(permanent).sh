...
# --- GIF ANIMADO (misma lógica ya validada en Batocera y en Recalbox Opción 1) ---
    ...
    case "$EVENTO" in
        rungame)
            if [ -f "$STATE_FILE" ]; then
                ...
                # Si hay gif(s) para este rom, lanzamos el streamer en segundo plano.
                # Si no hay ninguno, no pasa nada: la imagen estática de arriba se
                # queda tal cual en el panel.
                if [ -n "$JUEGO_LIMPIO" ]; then
                    rm -f "$STOP_FLAG"
