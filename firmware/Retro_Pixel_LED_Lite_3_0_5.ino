...
// ====================================================================
#define FIRMWARE_VERSION "3.0.5" // Hotfix
#define CURRENT_VERSION_NUM 305 // Versión numérica para comparar (2.1.0 -> 210)
...
void guardarConfigIni() {
    ...
    configFile.println(F("# Activa la recepcion de marquesinas desde Batocera: 0=OFF, 1=ON"));
    ...
    switch (estadoActual) {
        ...
        case ESTADO_SUBMENU_REPRODUCCION:
            if (cursorSubmenu == 0) {
                ...
                arcadeEnable = !arcadeEnable; // Cambia entre 0 y 1
...
void dibujarMenuOSD() {
    ...
    // ----------------------------------------------------------------
        ...
        snprintf(titleBuf, sizeof(titleBuf), "%s%d/2", msg("SUBMENU_REPRODUCCION", "titulo"), pagReproduccion + 1);
        printMenuCentrado(titleBuf, 0, display->color565(0, 255, 255), offset);
        display->drawLine(offset, 7, offset + offset, 7, display->color565(100, 100, 100));
        ...
        for (int i = 0; i < 3; i++) {
            ...
            switch (itemIndex) {
                ...
                case 2: display->printf("%s%s", msg("SUBMENU_REPRODUCCION", "arcade"), (arcadeEnable ? msg("ESTADOS", "si") : msg("ESTADOS", "no"))); break; 
...
void mostrarRelojLite(bool conTransicion = false) {
    ...
    if (arcadeEnable == 1 && estadoActual == ESTADO_ARCADE) return;
    ...
    while(millis() - startTime < (clockDuration * 1000UL) && !interrumpirReproduccion) {
        ...
        if (arcadeEnable == 1) server.handleClient(); 
...
bool buscarJuegoEnIndice_binaria(String sistema, String juego) {
    char rutaIndice[80];
    snprintf(rutaIndice, sizeof(rutaIndice), "/Arcade/%s.txt", sistema.c_str());
    ...
    String juegoBusca = juego;
    juegoBusca.toLowerCase();

    long bajo = 0;
    long alto = archivo.size();
    String linea;
    
    while (bajo < alto) {
        long medio = bajo + (alto - bajo) / 2;
        archivo.seek(medio);
        
        // Nos alineamos al inicio de la siguiente línea
        if (medio > 0) archivo.readStringUntil('\n');
        
        if (archivo.position() >= alto) break;

        //String linea = archivo.readStringUntil('\n');
        linea = archivo.readStringUntil('\n');
        linea.trim();

       String tempComp = linea; 
        tempComp.toLowerCase();

        if (tempComp == juegoBusca) { 
            archivo.close(); 
            return true; 
        }

        if (tempComp < juegoBusca) {
            bajo = archivo.position();
        } else {
            alto = medio;
        }
    }
...
void mostrarMarquesinaBMP(String path) {
...
void ejecutarModoGifLite() {
    ...
    if (gif.open(gifPath.c_str(), GIFOpenFile, GIFCloseFile, GIFReadFile, GIFSeekFile, GIFDraw)) {
        ...
        while (gif.playFrame(true, &delayMs)) {
            ...
            if (digitalRead(PIN_BOTON_MENU) == LOW || interrumpirReproduccion || (arcadeEnable == 1 && estadoActual == ESTADO_ARCADE)) {
                interrumpirReproduccion = true;
                break;
            }
...
void setup() {
    ...
    if (sdOk) {
        ...
        if (wifiEnable == 1 && !entrarFTP && wifi_ssid[0] != '\0') { 
            ...
            if(WiFi.status() == WL_CONNECTED) {
                ...
                // C. Si Arcade está habilitado mantenemos WiFi conectado
                if (arcadeEnable == 1) {
                Serial.println(F("[WIFI] Modo Arcade detectado: Manteniendo conexión activa."));
                } else {
                    Serial.println(F("[WIFI] Modo Arcade OFF: Desconectando tras actualización."));
                    WiFi.disconnect(true);
                    WiFi.mode(WIFI_OFF);
                    delay(500);

                    Serial.print(F("[MEMORIA] RAM libre tras WiFi: %d bytes\n"));
    ...
    bool usarDoubleBuff = (doubleBuff == 1) && !entrarFTP && (arcadeEnable != 1) && (transitionEnable != 1);
