...
// ====================================================================
#define FIRMWARE_VERSION "3.1.0" // Control RetroPixel LED desde la APP e implementación de la función Arcade 2.0
#define CURRENT_VERSION_NUM 310 // Versión numérica para comparar (2.1.0 -> 210)
#define GITHUB_VERSION_URL "https://github.com/fjgordillo86/RetroPixelLED-Lite/raw/refs/heads/main/docs/version.json"
#define GITHUB_IDIOMAS_API_URL "https://api.github.com/repos/fjgordillo86/RetroPixelLED-Lite/contents/Contenido%20SD/idioma"
...
void mostrarTextoScroll() {
    ...
    display->setFont(&fuente8pt7b_Bold);
    display->setTextSize(1);

    // Solo reconvertimos (UTF8->1 byte) y remedimos el texto cuando cambia el mensaje,
    // no en cada paso del scroll.
    static String ultimoMensajeCacheado = "";
    if (textoScrollMsg != ultimoMensajeCacheado) {
...
int descargarIdiomasGitHub(String &resumen) {
    if (WiFi.status() != WL_CONNECTED) { resumen = "Sin WiFi"; return 0; }
    ...
    if (!http.begin(client, GITHUB_IDIOMAS_API_URL)) {
        resumen = "No se pudo conectar a GitHub";
        return 0;
    }

    int code = http.GET();
    if (code != 200) {
        resumen = "GitHub respondio " + String(code);
        http.end();
        return 0;
    }

    DynamicJsonDocument doc(8192); // tamaño variable segun cuantos archivos haya en la carpeta
    DeserializationError err = deserializeJson(doc, http.getStream());
    http.end();
    if (err) { resumen = "JSON de GitHub invalido"; return 0; }

    if (!SD.exists("/idioma")) SD.mkdir("/idioma");

    int descargados = 0, fallidos = 0;

    for (JsonObject item : doc.as<JsonArray>()) {
        const char* nombre       = item["name"];
        const char* tipo         = item["type"];
        const char* urlDescarga  = item["download_url"];
        if (!nombre || !tipo || !urlDescarga) continue;
        if (String(tipo) != "file") continue;

        String nombreStr = String(nombre);
        if (!nombreStr.endsWith(".json")) continue;

        if (http.begin(client, urlDescarga)) {
            int codeArchivo = http.GET();
            if (codeArchivo == 200) {
                String ruta = "/idiomas/" + nombreStr;
